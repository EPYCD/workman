...
Filters can be combined; they're AND-ed together:
  --ready          ready to start, as judged by the server: in Todo, not
                   done, unassigned, no unfinished blocker and no owned path
                   leased by another task ("veans ready" explains the why-nots)
...
func runList(cmd *cobra.Command, rt *runtime, f *listFlags) ([]*client.Task, error) {
	...
	if f.ready {
		// Readiness is decided server-side so every agent and the board agree;
		// --filter cannot be pushed down onto that endpoint and is refused.
		if f.filter != "" {
			return nil, output.New(output.CodeValidation, "--ready and --filter cannot be combined")
		}
